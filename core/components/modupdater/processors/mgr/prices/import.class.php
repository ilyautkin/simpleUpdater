...
class rldImportProcessor extends modProcessor {
    ...
    public function process() {
		...
		$key = 'modupdater/import/'.$object['filename'];
        ...
        if ($_POST['parsed']) {
            ...
            if (!empty($_FILES['csv-file']['name']) && !empty($_FILES['csv-file']['tmp_name'])) {
                ...
                if (!$this->hasErrors()) {
                    if ($data === false) {
                        $this->modx->error->addField('csv-file-btn', $this->modx->lexicon('modupdater_import_fileuploadfailed'));
                    } else {
                        $this->modx->cacheManager->set($key, $data);
                        $object['log'][] = $this->modx->lexicon('modupdater_import_file_parsed') . ' ' . count($data);
                    }
                }
            } else {
                $this->modx->error->addField('csv-file-btn', $this->modx->lexicon('modupdater_import_fileuploadfailed'));
