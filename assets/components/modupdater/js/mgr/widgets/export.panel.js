modUpdater.panel.Export = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'modupdater-export-panel';
    }
    Ext.apply(config, {
		...
		items: [{
            xtype: 'button',
            text: _('modupdater_export_start'),
            fieldLabel: _('modupdater_export_start'),
            name: 'start-export',
	...
	modUpdater.panel.Export.superclass.constructor.call(this, config);
};
Ext.extend(modUpdater.panel.Export, MODx.FormPanel, {
    _selectCSV: function() {
    ...
    _showFileName: function(e) {
        ...
        Ext.getCmp('modupdater-export-panel-export-log').body.dom.innerHTML = "";
        document.getElementById('modupdater-export-panel-export-log').style.display = "none";
...
Ext.reg('modupdater-export-panel', modUpdater.panel.Export);